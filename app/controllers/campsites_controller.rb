class CampsitesController < ApplicationController
  ...
  before_action :set_key_openweather_map, :rakuten_tent_get, :rakuten_hotel_get, :rakuten_books_get, only: %i[guidance]
  ...
  def mypage
    @bookmark_campsites = current_user.bookmarks.order(created_at: :desc).map(&:campsite)
  end
  ...
  def rakuten_tent_get
    ...
    rakuten_tent['Items'].first(20).each do |tent|
      get_tent << {
        name: tent['Item']['itemName'],
        url: tent['Item']['itemUrl'],
        image_url: tent['Item']['smallImageUrls'][0]['imageUrl'],
        price: tent['Item']['itemPrice'],
        score: tent['Item']['reviewAverage'],
        count: tent['Item']['reviewCount']
      }
    end
    @tents = get_tent.sample(4)
  end

  def rakuten_hotel_get
    api_key = Rails.application.credentials.rakuten[:key]
    begin
      ...
      rakuten_hotel['hotels'].first(count).each do |hotel|
        get_hotel << {
          ...
          image_url: hotel['hotel'][0]['hotelBasicInfo']['hotelImageUrl'],
          review: hotel['hotel'][0]['hotelBasicInfo']['reviewAverage'],
          review_count: hotel['hotel'][0]['hotelBasicInfo']['reviewCount'],
  ...
  def rakuten_books_get
    ...
    rakuten_books['Items'].first(20).each do |book|
      get_book << {
        ...
        url: book['Item']['itemUrl'],
        image_url: book['Item']['mediumImageUrl'],
        price: book['Item']['itemPrice'],
        sales: book['Item']['salesDate'],
